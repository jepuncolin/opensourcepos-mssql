...
class Sale extends CI_Model
{
	...
	public function get_info($sale_id)
	{
		...
		$sales_tax = 'ISNULL(SUM(sales_items_taxes.sales_tax), 0)';
		...
		$this->db->select('
				...
				MAX(ISNULL(payments.sale_cash_adjustment, 0)) AS cash_adjustment,
				MAX(ISNULL(payments.sale_cash_refund, 0)) AS cash_refund,
				' . "
				$sale_total AS amount_due,
				MAX(ISNULL(payments.sale_payment_amount, 0)) AS amount_tendered,
	...
	public function search($search, $filters, $rows = 0, $limit_from = 0, $sort = 'sales.sale_time', $order = 'desc', $count_only = FALSE)
	{
		...
		{
			$where .= 'CONVERT(DATE,sales.sale_time) BETWEEN ' . $this->db->escape($filters['start_date']) . ' AND ' . $this->db->escape($filters['end_date']);
		}
		else
		{
        ...
        */
		...
		$tax = 'ISNULL(SUM(sales_items_taxes.tax), 0)';
		$sales_tax = 'ISNULL(SUM(sales_items_taxes.sales_tax), 0)';
		$internal_tax = 'ISNULL(SUM(sales_items_taxes.internal_tax), 0)';
		$cash_adjustment = 'ISNULL(SUM(payments.sale_cash_adjustment), 0)';
        ...
        */
		$this->db->query(
				...
				SELECT sales_items_taxes.sale_id AS sale_id,
					...
					SUM(CASE WHEN sales_items_taxes.tax_type = 1 THEN sales_items_taxes.item_tax_amount ELSE 0 END) AS sales_tax INTO '.$this->db->dbprefix('sales_items_taxes_temp').
		...
		else
		{
			$this->db->select('
					sales.sale_id AS sale_id,
					MAX(CONVERT(DATE,sales.sale_time)) AS sale_date,
					MAX(sales.sale_time) AS sale_time,
					MAX(sales.invoice_number) AS invoice_number,
					MAX(sales.quote_number) AS quote_number,
	...
	public function get_payments_summary($search, $filters)
	{
		...
		if(empty($this->config->item('date_or_time_format')))
		{
			$this->db->where('CONVERT(DATE,sales.sale_time) BETWEEN ' . $this->db->escape($filters['start_date']) . ' AND ' . $this->db->escape($filters['end_date']));
		}
		else
		{
	...
	public function create_temp_table(array $inputs)
	{
		if(empty($inputs['sale_id']))
		{
			if(empty($this->config->item('date_or_time_format')))
			{
				$where = 'CONVERT(DATE,sales.sale_time) BETWEEN ' . $this->db->escape($inputs['start_date']) . ' AND ' . $this->db->escape($inputs['end_date']);
			}
			else
			{
        ...
        */
		$this->db->query(
				...
				' SELECT sales_items_taxes.sale_id AS sale_id,
					...
					SUM(ROUND(CASE WHEN sales_items_taxes.tax_type = 1 THEN sales_items_taxes.item_tax_amount ELSE 0 END, ' . $decimals . ')) AS sales_tax INTO '.$this->db->dbprefix('sales_items_taxes_temp').
				' FROM ' . $this->db->dbprefix('sales_items_taxes') . ' AS sales_items_taxes
				INNER JOIN ' . $this->db->dbprefix('sales') . ' AS sales
					ON sales.sale_id = sales_items_taxes.sale_id
				INNER JOIN ' . $this->db->dbprefix('sales_items') . ' AS sales_items
        ...
        */
		$this->db->query(
       			"IF OBJECT_ID('".$this->db->dbprefix('sales_items_temp')."') IS NOT NULL DROP TABLE ".$this->db->dbprefix('sales_items_temp'). 
                '
				SELECT
					MAX(CONVERT(DATE,sales.sale_time)) AS sale_date,
					MAX(sales.sale_time) AS sale_time,
					...
					$sale_total AS total,
					$sale_cost AS cost,
					($sale_subtotal - $sale_cost) AS profit INTO
					" .$this->db->dbprefix('sales_items_temp').'  
