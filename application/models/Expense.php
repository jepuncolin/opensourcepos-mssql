	...
	*/
	public function search($search, $filters, $rows = 0, $limit_from = 0, $sort = 'expense_id', $order = 'asc', $count_only = FALSE)
	{
		...
		if(empty($this->config->item('date_or_time_format')))
		{
            ...
            $this->db->where('convert(varchar(20),expenses.date, 111) BETWEEN ' . $this->db->escape($filters['start_date']) . ' AND ' . $this->db->escape($filters['end_date']));
		}
		else
		{
	...
	*/
	public function get_payments_summary($search, $filters)
	{
		...
		if(empty($this->config->item('date_or_time_format')))
		{
            ...
            $this->db->where('convert(varchar(10),date, 111) BETWEEN ' . $this->db->escape($filters['start_date']) . ' AND ' . $this->db->escape($filters['end_date']));
		}
		else
		{
