	...
	*/
	public function create_temp_table(array $inputs)
	{
		if(empty($inputs['receiving_id']))
		{
			...
			{
				$where = 'WHERE CONVERT(DATE,receiving_time) BETWEEN ' . $this->db->escape($inputs['start_date']) . ' AND ' . $this->db->escape($inputs['end_date']);
			}
			else
			{
        ...
        */
		$this->db->query(
			"IF OBJECT_ID('".$this->db->dbprefix('receivings_items_temp')."') IS NOT NULL DROP TABLE ".$this->db->dbprefix('receivings_items_temp'). 
			'	SELECT 
					MAX(CONVERT(DATE,receiving_time)) AS receiving_date,
					...
					MAX(item_cost_price * quantity_purchased * receivings_items.receiving_quantity ) AS cost INTO '.$this->db->dbprefix('receivings_items_temp').
