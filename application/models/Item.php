...
class Item extends CI_Model
{
	...
	*/
	public function search($search, $filters, $rows = 0, $limit_from = 0, $sort = 'items.name', $order = 'asc', $count_only = FALSE)
	{
		...
		{
			$this->db->where('convert(varchar(10),trans_date, 111) BETWEEN ' . $this->db->escape($filters['start_date']) . ' AND ' . $this->db->escape($filters['end_date']));
		}
		else
		{
		...
		if($attributes_enabled)
		{
			...
			$this->db->select("STRING_AGG(DISTINCT CONCAT('_', definition_id, attribute_value) ORDER BY definition_id ,'\') AS attribute_values");
			$this->db->select("STRING_AGG(DISTINCT CONCAT('_', definition_id, convert(varchar(10),attribute_date, 111)),'|') AS attribute_dtvalues");
			$this->db->select("STRING_AGG(DISTINCT CONCAT('_', definition_id, attribute_decimal) , '|') AS attribute_dvalues");
	...
	*/
	public function get_info($item_id)
	{
		...
		$this->db->select('STRING_AGG (attribute_value, \'|\') AS attribute_values');
		$this->db->select('STRING_AGG (attribute_decimal, \'|\') AS attribute_dvalues');
		$this->db->select('STRING_AGG (attribute_date , \'|\') AS attribute_dtvalues');
        ...
        */
		$query = $this->db->query(
			'SELECT
			items.*,b.attribute_values,b.attribute_dvalues,b.attribute_dtvalues '.
			' FROM ' . $this->db->dbprefix('items') . ' AS items
			INNER JOIN (' . 
            " SELECT  items.item_id,STRING_AGG (attribute_value, '|') AS attribute_values,STRING_AGG (attribute_decimal, '|') AS attribute_dvalues,STRING_AGG (attribute_date , '|') AS attribute_dtvalues
            FROM ". $this->db->dbprefix('items') . ' AS items LEFT JOIN '.$this->db->dbprefix('attribute_links').' AS links ON items.item_id=links.item_id LEFT JOIN '.$this->db->dbprefix('attribute_values').' AS c ON links.attribute_id=c.attribute_id '.
            ' where items.item_id='. $this->db->escape($item_id).
            ' group by items.item_id' .  
            ' ) b on items.item_id=b.item_id'.
			'  WHERE items.item_id = ' . $this->db->escape($item_id)
		);
	...
	*/
	public function get_multiple_info($item_ids, $location_id)
	{
		...
		$this->db->select("STRING_AGG(DISTINCT CONCAT('_', definition_id, attribute_value) ORDER BY definition_id ,'|') AS attribute_values");
		$this->db->select("STRING_AGG(DISTINCT CONCAT('_', definition_id, convert(varchar(10),attribute_date, 111)) ORDER BY definition_id ,'|') AS attribute_dtvalues");
		$this->db->select("STRING_AGG(DISTINCT CONCAT('_', definition_id, attribute_decimal) ORDER BY definition_id ,'|') AS attribute_dvalues");
        ...
        */
        $this->db->select('items.item_id');
		$this->db->select('MAX(company_name) AS company_name');
		$this->db->select("STRING_AGG(CONCAT('_', definition_id, attribute_value) ,'|') AS attribute_values");
		$this->db->select("STRING_AGG(CONCAT('_', definition_id, convert(varchar(10),attribute_date, 111)) ,'|') AS attribute_dtvalues");
		$this->db->select("STRING_AGG(CONCAT('_', definition_id, attribute_decimal) ,'|') AS attribute_dvalues");
		$this->db->select('MAX(quantity) as quantity');
		$this->db->from('items');
		$this->db->join('suppliers', 'suppliers.person_id = items.supplier_id', 'left');
		$this->db->join('item_quantities', 'item_quantities.item_id = items.item_id', 'left');
        ...
        $this->db->select('b.attribute_values');
        $this->db->select('b.attribute_dtvalues');
        $this->db->select('b.attribute_dvalues');
        $this->db->select('b.quantity');
        $this->db->from('items');
		$this->db->join('('.$subquery.' ) b','items.item_id=b.item_id');
		return $this->db->get();
	...
	*/
	...
	public function get_stock_search_suggestions($search, $filters = array('is_deleted' => FALSE, 'search_custom' => FALSE), $unique = FALSE, $limit = 25)
	{
		...
		if(!$unique)
		{
			...
			//Search by description
			//$this->db->select($this->get_search_suggestion_format('item_id, name, pack_name, description'));
            $this->db->select($this->get_search_suggestion_format('item_id, pack_name, description'));
			$this->db->from('items');
			$this->db->where('deleted', $filters['is_deleted']);
			$this->db->where_in('item_type', $non_kit); // standard, exclude kit items since kits will be picked up later
			$this->db->where('stock_type', '0'); // stocked items only
