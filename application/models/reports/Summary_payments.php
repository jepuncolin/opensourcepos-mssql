...
class Summary_payments extends Summary_report
{
	...
	public function getData(array $inputs)
	{
        ...
        $groupby = '(CASE sale_type WHEN ' . SALE_TYPE_POS . ' THEN \'' . $this->lang->line('reports_code_pos')
		...
		$select .= $groupby.' AS trans_type, ';
		$select .= 'COUNT(sales.sale_id) AS trans_sales, ';
		$select .= 'SUM(sumpay_items.trans_amount) AS trans_amount, ';
		$select .= 'ISNULL(SUM(sumpay_payments.total_payments),0) AS trans_payments, ';
		$select .= 'ISNULL(SUM(sumpay_payments.total_cash_refund),0) AS trans_refunded, ';
		$select .= 'SUM(CASE WHEN sumpay_items.trans_amount - ISNULL(sumpay_payments.total_payments,0) > 0 THEN sumpay_items.trans_amount - ISNULL(sumpay_payments.total_payments,0) ELSE 0 END) as trans_due ';
...
//		$this->db->group_by('trans_type');
	...
	protected function create_summary_payments_temp_tables($where)
	{
        ...
        */
		$this->db->query(
       			"IF OBJECT_ID('".$this->db->dbprefix('sumpay_taxes_temp')."') IS NOT NULL DROP TABLE ".$this->db->dbprefix('sumpay_taxes_temp'). 
                '
				SELECT sales.sale_id, SUM(sales_taxes.sale_tax_amount) AS total_taxes INTO '.$this->db->dbprefix('sumpay_taxes_temp'). 
...
'CREATE TEMPORARY TABLE IF NOT EXISTS ' . $this->db->dbprefix('sumpay_items_temp') .
        ...
        */
		$this->db->query(
       			"IF OBJECT_ID('".$this->db->dbprefix('sumpay_items_temp')."') IS NOT NULL DROP TABLE ".$this->db->dbprefix('sumpay_items_temp'). 
        
			' SELECT sales.sale_id, '. $trans_amount.' INTO '.$this->db->dbprefix('sumpay_items_temp') 
			. ' FROM ' . $this->db->dbprefix('sales').' AS  sales ' 
		...
		$this->db->query('UPDATE ' . $this->db->dbprefix('sumpay_items_temp')
			. ' SET trans_amount = trans_amount + ISNULL((SELECT total_taxes FROM ' . $this->db->dbprefix('sumpay_taxes_temp')
			. ' AS sumpay_taxes WHERE sale_id = sumpay_taxes.sale_id),0)');
        ...
        */
		$this->db->query(
			"IF OBJECT_ID('".$this->db->dbprefix('sumpay_payments_temp')."') IS NOT NULL DROP TABLE ".$this->db->dbprefix('sumpay_payments_temp'). 
            '
				...
				SUM(sales_payments.cash_refund) AS total_cash_refund INTO '.$this->db->dbprefix('sumpay_payments_temp').
				' FROM ' . $this->db->dbprefix('sales') . ' AS sales
