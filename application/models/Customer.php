	...
	public function get_stats($customer_id)
	{
		...
		$this->db->query(
			...
			'	SELECT
					...
					SUM(sales_items.quantity_purchased) AS quantity INTO '.$this->db->dbprefix('sales_items_temp').
		...
		$this->db->query("IF OBJECT_ID('".$this->db->dbprefix('sales_items_temp')."') IS NOT NULL DROP TABLE ".$this->db->dbprefix('sales_items_temp')); 
